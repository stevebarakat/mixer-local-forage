...
function TrackChannel({ track, trackId, channels, currentTracks }: Props) {
  ...
  const meters = useRef(
    Array(channels.length).fill(new Tone.Meter().toDestination())
  );

  const { send } = MixerMachineContext.useActorRef();

  const [currentTrackFx, setCurrentTrackFx] = useState(
    Array(fxNames.length).fill(new Tone.Volume())
  );
  ...
  useEffect(() => {
    ...
    channels[trackId].connect(Tone.Destination);
    currentTrackFx.forEach((ctf) => {
      ctf && channels[trackId].chain(ctf, Tone.Destination);
    });
  });
